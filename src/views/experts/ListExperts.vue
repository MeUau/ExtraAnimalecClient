...
export default {
  ...
  methods: {
    ...
    viewExpert(id) {
      ...
      this.$fire({
        ...
        imageUrl: require(`@/assets/avatars/${
          this.getExpertLevelByPoints(expert.gamification.points).avatar
        }.png`),
        imageWidth: 150,
        imageHeight: 150,
    ...
    generateTemplate(expert) {
      return `
          <h5>${expert.gamification.points} pontos (${
        this.getExpertLevelByPoints(expert.gamification.points).name
      })</h5>
          <p>${expert.description}</p>
          <p>
          <b>Nome:</b> ${expert.name} <br>
          <b>Tipo de utilizador:</b> ${
            expert.type === "admin" ? "Administrador" : "Utilizador normal"
          } <br>
          <b>Data de registo:</b> ${this.formatDate(
            expert.registration_date
          )} <br>
