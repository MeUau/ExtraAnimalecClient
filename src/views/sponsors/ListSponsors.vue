...
export default {
  ...
  methods: {
    ...
    viewSponsor(id) {
      ...
      this.$fire({
        ...
        imageUrl: require(`@/assets/avatars/${
          this.getSponsorLevelByPoints(sponsor.gamification.points).avatar
        }.png`),
        imageWidth: 150,
        imageHeight: 150,
    ...
    generateTemplate(sponsor) {
      return `
          <h5>${sponsor.gamification.points} pontos (${
        this.getSponsorLevelByPoints(sponsor.gamification.points).name
      })</h5>
          <p>${sponsor.description}</p>
          <p>
          <b>Nome:</b> ${sponsor.name} <br>
          <b>Tipo de utilizador:</b> ${
            sponsor.type === "admin" ? "Administrador" : "Utilizador normal"
          } <br>
